...
fn main() {
    ...
    match env::var("LDAP_SOCKET_PATH") {
        Err(e) => log::error!("make sure to appropriately set the `LDAP_SOCKET_PATH` env var: {}", e),
        Ok(val) => {
            log::debug!("Connecting to the socket path: {}", &val);

            let socket_path = urlencoding::encode(&val);

            let ldap_url = CString::new(format!("ldapi://{}/", socket_path)).expect("CString::new failed");
            ...
            if result != LDAP_SUCCESS as i32 {
                log::error!("LDAP initialization failed: {}", unsafe {
            ...
            let mech = CString::new("EXTERNAL").expect("CString::new failed");
